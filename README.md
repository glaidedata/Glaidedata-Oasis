![docker image](https://github.com/glaidedata/Glaidedata-Oasis/actions/workflows/docker-publish.yml/badge.svg)
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/glaidedata/Glaidedata-Oasis.git
    cd Glaidedata-Oasis
    ```
    ...
    ```sh
    curl-L -o Glaidedata-Oasis.zip "https://github.com/glaidedata/Glaidedata-Oasis/archive/main.zip"
    unzip Glaidedata-Oasis.zip
    cd Glaidedata-Oasis
    ```
...
your `docker-compose.yaml` with `ghcr.io/glaidedata/glaidedata-oasis:main` for the services
...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/glaidedata/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
```sh
docker pull ghcr.io/glaidedata/glaidedata-oasis/jupyter:main
```
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/glaidedata/Glaidedata-Oasis/manifests/main": unauthorized`
